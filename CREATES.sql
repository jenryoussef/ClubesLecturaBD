...
CREATE OR REPLACE VIEW V_REUNIONES_BIMESTRE AS SELECT HA.ID_CLUB_MEMB ID_CLUB, HA.ID_LECTOR,
    EXTRACT(YEAR FROM CR.F_REUNION) ANIO,
    CEIL(EXTRACT(MONTH FROM CR.F_REUNION)/2) BIMESTRE,
    COUNT(*) TOTAL_REUNIONES
FROM HIST_ASIGNACIONES HA, CAL_REUNIONES CR
...
CREATE OR REPLACE VIEW vista_miembros_activos (id_club, id_lector, f_ing_club) AS
