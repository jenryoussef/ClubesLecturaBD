-- registras un pago
EXECUTE ADFJ_PRC_REGISTRAR_PAGO(&ID_CLUB, &ID_LECTOR, TO_DATE('&FECHA_ING', 'DD-MM-YYYY'));
...
VALUES (&id_club, &id_lector, SYSDATE);
...
-- si se ingresa el lector con id 6 este comando NO falla porque el lector con ID 6 fallo en el club 3, pero no tiene historial en el club 1.
 INSERT INTO ADFJ_HIST_MEMBRESIAS (id_club, id_lector, f_ing_club) 
VALUES (&id_club, &id_lector, SYSDATE);


-- registro del segundo pago del lector con id 4 en el club 1 (que cobra membresia)
EXECUTE ADFJ_PRC_REGISTRAR_PAGO(&id_club, &id_lector, TO_DATE('&fecha_ingreso','DD-MM-YYYY'));

-- si se ingresa club con id 2 falla porque este club con id 2 no cobra membresias
EXECUTE ADFJ_PRC_REGISTRAR_PAGO(&id_club, &id_lector, TO_DATE('&fecha_ingreso','DD-MM-YYYY'));
